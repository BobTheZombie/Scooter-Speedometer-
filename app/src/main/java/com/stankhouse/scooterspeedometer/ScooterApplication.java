...
public class ScooterApplication extends Application implements Application.ActivityLifecycleCallbacks {
    ...
    @Override public void onCreate(){super.onCreate();getSharedPreferences("speedometer",MODE_PRIVATE).edit().putBoolean("app_visible",false).apply();CopilotAlertQueue.get(this).setForeground(false);registerActivityLifecycleCallbacks(this);}
    ...
    private void setVisible(boolean visible){getSharedPreferences("speedometer",MODE_PRIVATE).edit().putBoolean("app_visible",visible).apply();CopilotAlertQueue.get(this).setForeground(visible);sendBroadcast(new Intent(MediaAccessService.ACTION_APP_VISIBILITY_CHANGED).setPackage(getPackageName()));}
