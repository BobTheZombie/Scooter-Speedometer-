...
/** Bounded on-demand disk cache for OpenNAV+ map tiles. It never prefetches tiles. */
final class MapTileCache {
    private static final long FRESH_MS=7L*24L*60L*60L*1000L,MAX_BYTES=120L*1024L*1024L;
    private final File directory;private long lastPrune;
    MapTileCache(Context context){directory=new File(context.getCacheDir(),"opennav_tiles");directory.mkdirs();}
    ...
    private synchronized void prune(long now){if(now-lastPrune<60000)return;lastPrune=now;File[] files=directory.listFiles((d,n)->n.endsWith(".png"));if(files==null)return;long total=0;for(File f:files)total+=f.length();if(total<=MAX_BYTES)return;Arrays.sort(files,Comparator.comparingLong(File::lastModified));for(File f:files){long size=f.length();if(f.delete())total-=size;if(total<=MAX_BYTES*.85)break;}}
